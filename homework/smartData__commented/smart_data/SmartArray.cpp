...
int SmartArray::instance = 0; // @@ Not called for in spec, but... sure!

SmartArray::SmartArray():data(nullptr),_size(0)
{
    instance+=1;
    count = instance; // @@ Not a bad way to tag SmartArray instances for debugging.
} // default constructor
...
// @@ Nice! But overkill! Not required by spec, but glad you're having fun with it!
...
SmartArray& SmartArray::setData(int* arr, const int size)
{
    ...
    catch (bad_alloc) // @@Well, there's no chance of this tripping, since no allocations are performed in the "try" block.
    {
...
SmartArray& SmartArray::insert(const int fromPos, const SmartArray &fromArray, const int howMany, const int toPos)
{
	...
	// @@ Instead of complex logic in this loop, try
	// just having two separate simpler loops (or just a couple of memcpy's)
    for(int i=0; i<newSize; i++, c++) {
		//@@ Silent bug here. data[c] will access beyond range of 'data' once i >= _size
...
{
	//@@ Would be nice to trap error conditions, pos+howMany > _size, pos < 0, etc.
    int newSize = _size-howMany; //add block to size
    int * newArray = new int[newSize]; //create new array with more space

    int a = 0;

	// @@ Again, simpler to understand and more efficient if you
	// have separate simpler loops. 
...
{
	// @@ Oh... wait.... your positions are 1-based? Terrible idea!
	// Get accustomed to 0-based indexing. Much easier to understand.
	// No silly math fixups involed! Also, c++ programmers just don't do that.
    return data[pos-1];
...
void SmartArray::empty()
{
    if(!isEmpty()) {
        delete[] data;
		// @@ probably also a good idea to set data = nullptr.
	}
    _size = 0;
} // remove all items
...
string SmartArray::toString() const
{
    ...
    for(int i=0; i<_size; ++i) {
		// @@ Bug: This will generate all kinds of crazy strings when the
		// data[i] value is not in the 0-9 range.
		// just do str += data[i], or formatted output via ostrstream
		// and lean on your << operator.
